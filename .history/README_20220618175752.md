...
On GNU/Linux and potentially MacOS and BSD (however untested and unsupported), the bash script kierancli-install.sh
will automatically download, install and update kierancli to its latest version on Git.

The program will be installed to ~/.local/share/kierancli/bin however the location can be easily changed by editing the script.
