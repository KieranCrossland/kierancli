...
fn run_rs_mode() {
    ...
    match rustcommand.as_str().trim() {
        ...
        "mode rust" => prompt();run_rs_mode(),
        "mode gitclone" => gitclone(),
        "ls" => ls();prompt(),
        "clear" => print!("{esc}[2J{esc}[1;1H", esc = 27 as char);//ANSI sequence clears screen.


        _ => red_ln!("Command not found."),
    }
        ...
        print!("{esc}[2J{esc}[1;1H", esc = 27 as char); //""{esc}[2J{esc}[1;1H", esc = 27 as char //ANSI sequence clears screen.
        prompt();
